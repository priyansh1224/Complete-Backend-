...
const ai = new GoogleGenAI({ apiKey: "AIzaSyB3rjujVD8m8nUuSRDqSFz2A8xUoiabQEg"});
...
async function getWeather(location) {
    ...
    for(const {city,date} of location){
       ...
       {
        const response =  await fetch(`http://api.weatherapi.com/v1/future.json?key=242fa5811b52412f86861719252109&q=${city}`);
        const data = await response.json();
        weatherInfo.push(data);
       }
       else{
        const response =  await fetch(`http://api.weatherapi.com/v1/future.json?key=242fa5811b52412f86861719252109&q=${city}&dt=${date}`);
        const data = await response.json();
        weatherInfo.push(data);
       }
    }
